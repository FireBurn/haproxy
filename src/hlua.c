...
__LJMP static int hlua_channel_set(lua_State *L)
{
	...
	if (len > ci_data(chn) || channel_input_closed(chn))
		lua_pushinteger(L, -1);
	else {
...
__LJMP static int hlua_applet_tcp_set_priv(lua_State *L)
{
	...
	struct hlua *hlua = hlua_stream_ctx_get(s, luactx->appctx->ctx.hlua_apptcp.hlua->state_id);
...
__LJMP static int hlua_applet_http_set_priv(lua_State *L)
{
	...
	struct stream *s = luactx->htxn.s;
	struct hlua *hlua = hlua_stream_ctx_get(s, luactx->appctx->ctx.hlua_apphttp.hlua->state_id);
